...
export type ProfileInput = {
    ...
    skills: string[];
    volunteer_work: VolunteerWorkInput[];
  };
 ...
 {/* async function fetchProfilesJobsAndStages(orgId: string, publicIdentifiers: string[]): Promise<any[]> {
    // Convert publicIdentifiers from an array to a comma-separated string
    const publicIdentifiersString = publicIdentifiers.join(',');
  
    // Use the environment variable NEXT_PUBLIC_API_URL to point to the correct endpoint
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/profiles/profilesJobsAndStages?orgId=${orgId}&publicIdentifiers=${publicIdentifiersString}`);
...
interface FormState {
  ...
  minimumCategoryExperience: number; // Change this line
  isOpen: boolean;
...
const clearCandidateType = () => {
 ...
 {/* console.log('Session:', session);
  ...
  const handleSaveProfile = async (sharedProfileId: number, jobIds: number[], orgId: string, token: string) => {
    ...
    // Find the shared profile in the results array
    const sharedProfile = results.profiles.find(profile => profile.id === sharedProfileId);
  
    if (sharedProfile) {
      let profileId;
  
      // Fetch the profile with the matching public identifier
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/profiles/public_identifier/${sharedProfile.public_identifier}`, {
        headers: { 
      ...
      if (existingProfile && existingProfile.id) {
        // If the profile already exists, use its profileId
        profileId = existingProfile.id;
      } else {
        // If the profile doesn't exist, create a new one
        profileId = await createProfileFromSharedProfile(sharedProfile, sharedProfile.isProfile);
      }
  
      // Prepare the data to be saved
      const profileData = {
        stage: 'ADDED',
        comfortZoneScore: sharedProfile.comfortZoneScore,
        categoryScore: sharedProfile.categoryScore,
        overallScore: sharedProfile.overallScore,
        stabilityScore: sharedProfile.stabilityScore,
        recentCategoryScore: sharedProfile.recentCategoryScore,
      ...
      try {
        ...
        // After the profile is successfully saved to a job, update the `results` state
        setResults(prevResults => {
          return {
            ...prevResults,
            profiles: prevResults.profiles.map(p => p.id === sharedProfile.id ? { ...p, isProfile: true } : p)
          };
        });
  
        // Uncheck the checkbox for the saved profile
        setSelectedProfiles(prevState => prevState.filter(id => id !== sharedProfileId));
  
        // Unselect the job for the saved profile
        setSelectedJobsByProfile(prevState => {
          const newState = { ...prevState };
          delete newState[sharedProfileId];
          return newState;
        });
  ...
  const createProfileFromSharedProfile = async (profile: ProfileInput, isProfile: boolean) => {
    try {
      ...
      // If the profile already exists in the database, simply return
      if (isProfile) {
        return;
      }
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/profiles/createFromSharedProfile`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
     ...
     {/* console.log('Saved profile:', data); */}
      ...
      return data.id; // Replace 'id' with the actual ID field name in your response
    } catch (error) {
      console.error('An error occurred:', error);
    }
  };

 // Handler function for the "Create Profiles" button
const handleCreateProfilesFromSelected = async () => {
  ...
  for (const profileId of selectedProfiles) {
    ...
    if (profile) {
      await createProfileFromSharedProfile(profile, profile.isProfile);
      profilesCreated++;
    }
  }

  if (profilesCreated > 0) {
    window.alert(`${profilesCreated} profile(s) created.`);
  } else {
    window.alert('No profiles were created.');
  }
  ...
  const addCandidateToJobAndUpdateStage = async (profileId: number, jobId: number, stage: string, comfortZoneScore: number, categoryScore: number, overallScore: number, stabilityScore: number, recentCategoryScore: number, orgId: string, token: string) => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidateStages/${profileId}/${jobId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`, // Add the Authorization header
    ...
    if (!response.ok) {
      throw new Error('Failed to add candidate to job and update stage');
    }
  
    const data = await response.json();
   {/* console.log('Added candidate to job and updated stage:', data); */}
  };
...
const [showEducation, setShowEducation] = useState<Record<string, boolean>>({});
  ...
  const [profiles, setProfiles] = useState<any[]>([]); // Add this line at the beginning of your component
  const profileIdNumber = Number(profileId);
  const [isSingleColumnShared, setIsSingleColumnShared] = useState<boolean>(false);
  const [isFormVisible, setIsFormVisible] = useState(false);


  useEffect(() => {
    const savedIsSingleColumnShared = localStorage.getItem('isSingleColumnShared');
    if (savedIsSingleColumnShared) {
      setIsSingleColumnShared(JSON.parse(savedIsSingleColumnShared));
    }
  }, []);

  useEffect(() => {
    localStorage.setItem('isSingleColumnShared', JSON.stringify(isSingleColumnShared));
  }, [isSingleColumnShared]);
...
{/* useEffect(() => {
  if (results && orgId) { // Check if orgId is not null
    const publicIdentifiers = results.profiles.map(profile => profile.public_identifier);

    fetchProfilesJobsAndStages(orgId, publicIdentifiers)
      .then(fetchedProfiles => {
        setProfiles(fetchedProfiles);
      })
      .catch(error => {
        console.error('Error fetching profiles jobs and stages:', error);
      });
  }
}, [results, orgId]); // Run the effect when the results or orgId change */}
...
const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (name === 'enterprise') {
    ...
    if (checked) {
      ...
      // Remove 'AND midmarket, Mid-market, commercial' from the form
      setForm(prevForm => ({
        ...prevForm,
        experienceTitle: prevForm.experienceTitle.replace(/ AND Midmarket, Mid-market, Commercial|Midmarket, Mid-market, Commercial/g, '')
      }));
    }
  }
};
  ...
  for (const profileId of selectedProfiles) {
    ...
    if (profile) {
      ...
      if (existingProfile && existingProfile.id) {
        ...
        // If the profile doesn't exist, create a new one
        newProfileId = await createProfileFromSharedProfile(profile, profile.isProfile);
      }
      ...
      try {
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidateStages/${newProfileId}/${selectedJob}`, {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        setResults(prevResults => {
          return {
            ...prevResults,
            profiles: prevResults.profiles.map(p => p.id === profile.id ? { ...p, isProfile: true } : p)
          };
        });
...
useEffect(() => {
  ...
  if (page) {
    ...
    const loadFromIndexedDB = async () => {
      const savedCurrentPage = await get('OtherPageCurrentPage');
...
useEffect(() => {
  const loadFromIndexedDB = async () => {
    const savedResults = await get('OtherPageResults');
...
const fetchData = async (page: number): Promise<Results> => {
  ...
  try {
    const { sortStability, minExperience, maxExperience, minCompanies, ...filledFormFields } = form;
    ...
    const profileResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/profiles/profilePublicIdentifiers`, { headers });
    const profilePublicIdentifiers = profileResponse.data;

    const minimumCategoryExperience = form.minimumCategoryExperience || 0;

    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/profiles/sharedRawProfiles`, { 
      params: { 
        ...filledFormFields, 
    ...
    let data = Array.isArray(response.data.sharedRawProfiles) ? response.data.sharedRawProfiles.map((profile: any) => {
      const isProfile = profilePublicIdentifiers.includes(profile.public_identifier);
    ...
    // Remove the client-side filters for minExperience, maxExperience, and minCompanies
    // as these filters are now applied on the server side
...
const handleSubmit = async (event: FormEvent) => {
  ...
  {/* // Check if country is selected
    if (!form.country) {
      alert('Please select a country, as well as a software category and/or candidate type before searching.');
      return;
    } */}
  
     // Check if category or candidate type is selected
    if (form.selectedCategories.length === 0 && !form.selectedCandidateType) {
      alert('Please select a category or candidate type before searching.');
      return;
    } 
  ...
  setFormSubmitted(true);
    ...
    try {
      const { sortStability, minimumCategoryExperience, ...filledFormFields } = form;
...
// Fetch all profile public identifiers
  const profileResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/profiles/profilePublicIdentifiers`, { headers });

  const profilePublicIdentifiers = profileResponse.data;

  // Include minimumCategoryExperience in the axios request
  const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/profiles/sharedRawProfiles`, { 
    params: { 
      ...filledFormFields, 
      selectedCategoryNames, 
      ...
      page: currentPage, 
      pageSize: 25,
      minExperience: form.minExperience, // Add this line
      maxExperience: form.maxExperience, // Add this line
      minCompanies: form.minCompanies, // Add this line
      minimumRecentScore: form.minimumRecentScore, // Add this line

    }, 
    headers 
  });
  // Add a property to each shared raw profile that indicates whether it is also a profile
  let data = Array.isArray(response.data.sharedRawProfiles) ? response.data.sharedRawProfiles.map((profile: any) => {
    const matchingProfile = profilePublicIdentifiers.find((p: { id: number, public_identifier: string }) => p.public_identifier === profile.public_identifier);
    const isProfile = Boolean(matchingProfile);
    const id = matchingProfile ? matchingProfile.id : null;
    return {
      ...profile,
      isProfile,
      id,
...
{/* console.log('Data before Promise.all:', data);
console.log('profilePublicIdentifiers:', profilePublicIdentifiers); */}


data = await Promise.all(data.map(async (profile: any) => {
    // Inside Promise.all
    {/* console.log('Current profile public_identifier:', profile.public_identifier);
    console.log('isProfile before:', profilePublicIdentifiers.includes(profile.public_identifier)); */}
  const categoryScore = await calculateCategoryScore(profile.experiences, selectedCategoryNames);
  const stabilityScore = calculateStabilityScore(profile, form.stableTenure);
  const recentCategoryScore = await calculateRecentCategoryScore(profile.experiences, selectedCategoryNames, form.recentYears, form.yearsInCompany);
  ...
  const profileInfo = profilePublicIdentifiers.find((p: { public_identifier: string }) => p.public_identifier === profile.public_identifier);    const isProfile = Boolean(profileInfo);
    const profileId = isProfile ? profileInfo.id : null;
...
{/* console.log('Data after Promise.all:', data); */}
    ...
    let filteredData = Array.isArray(response.data.sharedRawProfiles) ? response.data.sharedRawProfiles.map((profile: any) => {
      const isProfile = profilePublicIdentifiers.includes(profile.public_identifier);
      return {
        ...profile,
        isProfile,
    ...
    // Sorting logic based on stability score
{/* if (sortStability) {
  filteredData = filteredData.sort((a: { stabilityScore: number }, b: { stabilityScore: number }) => b.stabilityScore - a.stabilityScore);
} */}
...
const toggleMoreInfo = (index: number) => {
  setResults((prevResults) => {
    const updatedResults = [...prevResults.profiles];
    updatedResults[index].isExpanded = !updatedResults[index].isExpanded;
    return { ...prevResults, profiles: updatedResults };
  });
};
...
<button 
  ...
  style={{ backgroundColor: showDivCategory ? '#E07A5F' : '#3D4f6B', color: '#fff' }}
...
<div>
  <button 
    ...
    style={{ backgroundColor: isFormVisible ? '#E07A5F' : '#3D4f6B', color: '#fff' }}
    className="w-full text-white rounded-md p-1 mt-1 text-xxs sm:text-xs font-bold pt-2"
  >
...
<button 
  ...
  style={{ backgroundColor: '#3D4f6B', color: '#fff' }}
...
{results && results.profiles && results.profiles.length > 0 && (
  <div className="fixed bottom-4 right-4 bg-white p-1 rounded-sm shadow-md z-10 w-36 sm:w-28 md:w-48 lg:w-64 border border-gray-300">
...
<div className="container mx-auto px-4 md:px-15">
  <div className="flex justify-end mb-4">
    <button
      className={`md:block hidden mr-2 ${isSingleColumnShared ? 'bg-blue-700 text-white' : 'bg-white text-blue-700'} p-2 rounded`}
      onClick={() => setIsSingleColumnShared(true)}
    >
      <FaRegWindowMaximize />
    </button>
    <button
      className={`md:block hidden ${!isSingleColumnShared ? 'bg-blue-700 text-white' : 'bg-white text-blue-700'} p-2 rounded`}
      onClick={() => setIsSingleColumnShared(false)}
    >
      <FaThLarge />
    </button>
  </div>
  <div className={`w-full grid grid-cols-1 ${!isSingleColumnShared ? 'md:grid-cols-3' : ''} gap-4 overflow-auto`}>
...
const fetchProfile = async (event: React.MouseEvent) => {
  event.stopPropagation();
    ...
    const handleAddToTalentPool = (e: React.MouseEvent) => {
      e.stopPropagation();
      createProfileFromSharedProfile(profile, profile.isProfile)
        .then(() => {
          alert('Candidate has been saved to the Talent Pool');
          // Assuming `setResults` is the state update function and `results` is the current state
          setResults(prevResults => {
            return {
              ...prevResults,
              profiles: prevResults.profiles.map(p => p.id === profile.id ? { ...p, isProfile: true } : p)
        ...
        .catch(() => alert('There was an error saving the candidate to the Talent Pool'));
    };
    ...
    const handleSave = (e: React.MouseEvent) => {
      ...
      if (token && orgId) {
        handleSaveProfile(profile.id, selectedJobsByProfile[profile.id] || [], orgId, token)
          ...
          .catch(error => {
            console.error('Error saving profile to job:', error);
          });
      } else {
  ...
  <div 
    key={profile.id} 
    className={`p-0.5 sm:p-2 ${isSingleColumnShared ? 'sm:w-3/4 sm:mx-auto px-1 sm:px-4' : ''} flex flex-col`}
  >
    <div 
      className={`p-1 sm:p-4 flex flex-col bg-white rounded-lg cursor-pointer flex-grow ${profile.isProfile ? 'shadow-lg' : ''}`} 
      style={profile.isProfile ? { boxShadow: '0 0 10px #39ff14' } : {}}
      onClick={() => {
    ...
    >
      <div className="flex items-start mb-1 sm:mb-4">
        ...
        <div className="flex-1">
          <div className="text-xxs sm:text-sm font-bold text-gray-600 mb-0.5">{profile.full_name}</div>
          <div className="text-xxs sm:text-xs font-bold text-gray-600 mb-0.5">
  ...
  >
    <img src="/logos/LI-In-Bug.png" alt="LinkedIn Profile" className="w-3 h-2 sm:w-5 sm:h-4" />
      ...
      <div className="mb-1 sm:mb-4">
        {profile.categoryScore && Object.keys(profile.categoryScore.categoryExperience).map((category) => (
          <div key={category} className="inline-block mr-1 mb-0.5 sm:mb-2">
            <span style={{ backgroundColor: '#81B29A', color: '#fff' }} className="badge text-xxs sm:text-xs px-2 py-0.5 rounded-full">
              {category}
            </span>
...
{profile.recentCategoryScore > 0 && (
  ...
  {profile.isProfile ? (
    <button onClick={fetchProfile} className="mt-0.5 sm:mt-2 mb-0.5 sm:mb-2 bg-green-200 text-green-700 text-xs sm:text-sm px-1 py-0.5 rounded">
      In your Talent Pool - Click to View
    </button>
  ) : (
          <button onClick={handleAddToTalentPool} className="mt-0.5 sm:mt-2 mb-0.5 sm:mb-2 bg-blue-200 text-blue-700 text-xs sm:text-sm px-1 py-0.5 rounded">
            Add to Talent Pool
          </button>
        )}
        ...
        <div className="w-full flex flex-col justify-between">
          <div className="flex justify-end">
            <div className="flex items-center" onClick={(e) => e.stopPropagation()}>
              ...
              />
              <label htmlFor={`profile-${profile.id}`} className="text-xxs sm:text-sm">Select</label>
            </div>
          </div>
        </div>
      </div>
